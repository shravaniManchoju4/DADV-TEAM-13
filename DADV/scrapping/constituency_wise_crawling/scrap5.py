...
import csv
with open("data/S29.csv", 'w') as f:
        page = requests.get("http://eciresults.nic.in/ConstituencywiseS2653.htm?ac=53")
        soup = BeautifulSoup(page.content, 'html.parser');
        div = soup.find('div');
        print(div)
        data = div.find('input', id="HdnTG", type="hidden").get('value').split(';');
        for i in range(0,len(data)-1):
            st=data[i].split(',')
            print(st)
            n = st[0]
            page = requests.get("http://eciresults.nic.in/ConstituencywiseS29" + n + ".htm?ac=" + n)
            soup = BeautifulSoup(page.content, 'html.parser');
